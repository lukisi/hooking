...
namespace SystemPeer
{
        ...
        if (task.has_prefix("add_identity,"))
        {
            ...
            ArrayList<int> arc_list_peer_id_num = new ArrayList<int>();
            {
                ...
                for (int i = 0; i < parts.length; i++)
                {
                    ...
                    if (parts2.length != 2) error("bad arc_list in task 'add_identity'");
                    {
                        int64 element;
    ...
    class AddIdentityTasklet : Object, ITaskletSpawnable
    {
        ...
        public void * func()
        {
            ...
            comm_skeleton_factory.start_stream_system_listen(st_listen_pathname, another_identity_data.local_identity_index);
            tasklet.ms_wait(1);
    ...
    bool schedule_task_add_identityarc(string task)
    {
        if (task.has_prefix("add_identityarc,"))
        {
            ...
            if (parts2.length != 2) error("bad arc_num+peer_id+peer_naddr_pos in task 'add_identityarc'");
            {
                int64 element;
